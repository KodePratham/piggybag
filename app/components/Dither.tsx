...
const RetroEffect = forwardRef<RetroEffectImpl, { colorNum: number; pixelSize: number }>((props, ref) => {
  ...
  const effect = React.useMemo(() => new RetroEffectImpl(), []);
...
function DitheredWaves({
  ...
  useFrame(({ clock, mouse }) => {
    ...
    if (enableMouseInteraction) {
      // Use Three.js normalized mouse coordinates and convert to pixel space
      const dpr = gl.getPixelRatio();
      const x = ((mouse.x + 1) / 2) * size.width * dpr;
      const y = ((1 - mouse.y) / 2) * size.height * dpr; // Flip Y to fix reversed interaction
      u.mousePos.value.set(x, y);
    }
  });
